...
`MoneyMonkey.sln`, na raiz deste diretório, referencia quatro projetos:
...
| `MoneyMonkey.Data` | `DbContext` do EF Core, entidades e repositórios |
...
- .NET 8 SDK
- PostgreSQL com o schema já criado (não há migrations do EF Core neste projeto — tabelas `users`, `credentials`, `categories`, `transactions` e os enums `user_type`, `transaction_type`, `payment_method` são esperados previamente no banco)
...
Os enums `UserType`, `TransactionType` e `PaymentMethod` são mapeados para tipos enum nativos do Postgres.
